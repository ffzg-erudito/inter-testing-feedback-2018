...
class Constants(BaseConstants):
    ...
    with open('fwt_gk1/gk_quiz1.csv', encoding = 'utf8') as questions_file:
        gk_quiz1_questions = list(csv.DictReader(questions_file))

    num_rounds = len(gk_quiz1_questions)
    

class Subsession(BaseSubsession):
    def creating_session(self):
        if self.round_number == 1:
            self.session.vars['gk_quiz1_questions'] = Constants.gk_quiz1_questions.copy()
            ## ALTERNATIVE DESIGN:
...
class Player(BasePlayer):
    ...
    def current_question(self):
        return self.session.vars['gk_quiz1_questions'][self.round_number - 1]
