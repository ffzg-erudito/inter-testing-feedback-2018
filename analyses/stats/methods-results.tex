...
conducted an ANOVA with interpolated activity type as the independent
...
\(p = 6.873\times 10^{-15}\)), but after taking into account the number
...
Cohen's \(U_{3, g_s}\) = 74.43\%, probability of superiority = 67.88\%).
...
warrants certain caveats. The \texttt{BayesFactor} package's default
...
intervals obtained using \texttt{HDInterval}
