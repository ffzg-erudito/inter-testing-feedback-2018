...
datHardNofeed %>% select(., -activityFactor) %>%
  mvn(., multivariatePlot = 'qq', mvnTest = 'hz', desc = T)
```
...
the distribution of the total number of intrusive distractors chosen. It is interesting
...
Furthermore, when the test is performed for each group separately, the departures
are declared to be non-significant in two out of three cases, the third case being
marginally significant. Therefore, we will assume that the data in each group
...
with conducting a linear discriminant analyis, as well as Roy-Bargmann's stepdown
analysis.

### Linear discriminant analysis

```{r lda}
datHardNofeed %>% count(., activityFactor) %>% pull(n) %>%
  {lda(activityFactor ~ totalCorrect + totalIntrusors,
      data = datHardNofeed,
      prior = c(.[1]/nrow(datHardNofeed), .[2]/nrow(datHardNofeed),
                .[3]/nrow(datHardNofeed)))} -> ldaModel
ldaModel
```

```{r}
ldaModel %>% predict(.) %>% pluck('x') %>% as.data.frame(.) %>%
  cbind(datHardNofeed) %>%
  ggplot(., aes(x = LD1, y = LD2, color = activityFactor)) +
