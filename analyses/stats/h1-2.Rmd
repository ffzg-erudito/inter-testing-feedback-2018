...
The analysis plan (`analysis-plan.md`) lists a TOST and Bayesian t-test to try to
find evidence of no difference between the two test groups. Since a difference was found
while conducting the planned contrasts, these analyises are no longer deemed
necessary. Furthermore, somewhere during the analysis process, we've concluded
that a Bayesian t-test would be fair, even if the difference between the two groups
were non-significant, since it has a point null hypothesis. A Bayesian test specifying
the region of practical equivalence would have been more appropriate.
