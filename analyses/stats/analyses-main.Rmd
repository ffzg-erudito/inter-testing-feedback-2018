...
(bootstrap\textsubscript{R = 10000} median = `r bootOut$t %>% median(.)`, \(BC_\alpha\) 95% CI =
...
(bootstrap\textsubscript{R = 10000} median = `r bootOutActivity$t %>% median(.)`, \(BC_\alpha\) 95% CI =
...
(bootstrap\textsubscript{R = 10000} median = `r bootOutFeed$t %>% median(.)`\footnote{
...
manovaStats(manovaModelInter, 'test', '^activityFactor:', test = 'Wilks'),
manovaStats(manovaModelInter, 'df', '^activityFactor:', test = 'Wilks'))`
(bootstrap\textsubscript{R = 10000} median = `r bootOutInt$t %>% median(.)`\footnote{
Same as footnote \ref{bca-ref}. \(BC_\alpha\) 95\% CI = \([`r ciOutInt$bca[4]`, `r ciOutInt$bca[5]`]\).}). Both the feedback and the interaction
...
Plots created using `ggplot2` \citep{wickham_ggplot2:_2016}. Bootstrap conducted
using the `boot` package \citep{canty_boot:_2017}. Methods and analyses written
using `rmarkdown` \citep{allaire_rmarkdown:_2019} and `knitr` \citep{xie_knitr:_2019}. The package `car`
\citep{fox_r_2011} was used to obtain type III sums of squares. `compute.es` \citep{re_compute.es:_2013}
was used to obtain effect sizes for contrasts.
`kableExtra` was used to help generate tables \citep{zhu_kableextra:_2019}.
Other utilities used are `tidyverse` \citep{wickham_tidyverse:_2017},
`magrittr` \citep{bache_magrittr:_2014}, `here` \citep{muller_here:_2017},
`conflicted` \citep{wickham_conflicted:_2018}, `psych` \citep{revelle_psych:_2018}.
