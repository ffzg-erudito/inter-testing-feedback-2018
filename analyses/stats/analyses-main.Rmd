...
```{r boxPlot, include = T, fig.align = 'center', fig.cap = '\\label{box1} Boxplots broken down by experimental condition and dependent variable, with overlayed raw scores.', fig.env = 'figure*', message = F, fig.pos = 'h'}
datHardNofeed %>%
...
(bootstrap (R = 10000) median = `r bootOut$t %>% median(.)`, \(BC_\alpha\) 95% CI =
...
datHardFeed %<>% mutate_at(., vars(activityFactor), as.factor)
```
...
test conditions (\(n\) = `r nrow(datHardFeed)`). To test these hypotheses, we first
...
ciOutFeed <- boot.ci(bootOut, index = 1)
```
...
The effect size, calculated as
\(\omega^2_{mult}\) = `r summary(manovaModelInter, test = 'Wilks') -> .;
omegaSquared(nrow(manovaModelInter$residuals), .$stats[1, 'Wilks'], .$stats[1, 'Df'])`
(bootstrap (R = 10000) median = `r bootOutFeed$t %>% median(.)`, \(BC_\alpha\) 95% CI =
[`r ciOutFeed$bca[4]`, `r ciOutFeed$bca[5]`]).

On the other hand, we find no effect of feedback ---
Pillai's V =
...
```{r}
Anova(manovaModelInter, type = '3') %>% pluck('SSPE') -> .e

Anova(manovaModelInter, type = '3') %>% pluck('SSPE') %>%
  {. + Anova(manovaModelInter, type = '3') %>% pluck('SSP') %>% pluck('activityFactor')} -> .h

det(.e) / det(.h)

Anova(manovaModelInter, type = '3') %>% pluck('SSP') %>% pluck('giveFeedback') -> .h
