...
# r2activity
X <- model.matrix(totalCorrect ~ activityFactor,
                  datHardFeed)

X <- X[, -1]

cancorYX <- cancor(x = X, y = Y)

r2activity <- 1 - (1 - cancorYX$cor^2)
...
f2m <- nrow(datHardFeed) - 3 - (2 + 1 + 3) * .5

f2v <- f2m * f2s + 1 - f2u / 2

f12activity <- lW^(-1/f2s) * (f2v / (f2v + f2u)) - 1

g <- (1 - r2full) / (1 - r2activity)

f22activity <- f12activity / g

# r2feedback
X <- model.matrix(totalCorrect ~ giveFeedback,
                  datHardFeed)
...
f12feedback <- lW^(-1/f2s) * (f2v / (f2v + f2u)) - 1

g <- (1 - r2full) / (1 - r2feedback)

f22feedback <- f12feedback / g

# r2inter
Xab <- model.matrix(totalCorrect ~ giveFeedback + activityFactor,
                    datHardFeed)
...
detYXab <- det(cor(cbind(Y, Xab)))

r2ab <- 1 - detYXab / (detY * detXab)

r2abPartial <- (r2full - r2ab) / (1 - r2ab)
...
f12inter <- lW^(-1/f2s) * (f2v / (f2v + f2u)) - 1
```
